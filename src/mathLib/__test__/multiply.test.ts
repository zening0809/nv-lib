...
describe('testmultiply', function () {
    xit('STRESS TEST - should return correct result', function(){
        let count = 100, err = 0;
        while(count-- > 0){
            const dividend = Math.round(Math.random()*(Math.pow(10, Math.floor(Math.random()*5))));
            const divisor = Math.round(Math.random()*(Math.pow(10, Math.floor(Math.random()*5))));
            //if(divisor==0)
                //continue;
            const jsOut = dividend*divisor;
            const myOut = parseFloat(multiply(dividend, divisor));
