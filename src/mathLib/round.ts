 ...
 * 
 */
export default function round(input: number | string, n: number = 0, mode=RoundingModes.HALF_EVEN): string {
...
function greaterThanFive(part: string, pre: string, neg: boolean, mode: RoundingModes): boolean {
...
function increment(part: any, c: number = 0): string {
    if (!c)
        c = 1;
