...
/** Used to store nv to test for bad extensions/shims. */
let nvBizarro = root.nvBizarro
...
const phantom = root.phantom,
    ...
    amd = root.define ? root.define.amd : undefined,
    args = toArgs([1, 2, 3]),
    ...
    strictArgs = (function(...argument: number[]): number[] { return argument }(1, 2, 3))
let params = argv
...
/** Stub functions. */
const stubA = function(): string { return 'a' },
    stubB = function(): string { return 'b' },
    stubC = function(): string { return 'c' }

const stubTrue = function(): boolean { return true },
    stubFalse = function(): boolean { return false }

const stubNaN = function(): any { return NaN },
    stubNull = function(): null { return null }

const stubZero = function(): number { return 0 },
    stubOne = function(): number { return 1 },
    stubTwo = function(): number { return 2 },
    stubThree = function(): number { return 3 },
    stubFour = function(): number { return 4 }
...
const filePath = (function(): any {
  let min = 2,
      result = params || []

  if (phantom) {
    min = 0
    result = params = phantom.args || import('system').args
  }
  const last = result[result.length - 1]
  result = (result.length > min && !/test(?:\.js)?$/.test(last)) ? last : '../index.ts'

  if (!amd) {
    try {
      result = import('fs').realpathSync(result)
    } catch (e) {}
...
const isNpm = isModularize && /\bnpm\b/.test(ui.buildPath)

/** Detect if running in PhantomJS. */
const isPhantom = phantom || (typeof root.callPhantom === 'function')
...
//     const stack = nvStable.last(arguments)
...
const coverage = root.__coverage__ || root[nvStable.find(nvStable.keys(root), function(key: string) {
  return /^(?:\$\$cov_\d+\$\$)$/.test(key)
})]

/** Used to test async functions. */
const asyncFunc = nvStable.attempt(function() {
  return Function('return async () => {}')
})

/** Used to test generator functions. */
const genFunc = nvStable.attempt(function() {
 ...
 */
const whitespace = nvStable.filter([
...
function(chr: string) { return /\s/.exec(chr) })
.join('')
 ...
 */
function CustomError(message: any): void {
  this.name = 'CustomError'
  this.message = message
}

CustomError.prototype = nvStable.create(Error.prototype, {
 ...
 */
function emptyObject(object: any): void {
  nvStable.forOwn(object, function(value: any, key: string | number, object: any) {
    delete object[key]
  })
}
 ...
 */
function getUnwrappedValue(wrapper: any): any {
    let index = -1
 ...
 */
function interopRequire(id: string): any {
  const result = import(id)
 ...
 */
function setProperty(object: any, key: string, value: any): any {
  try {
 ...
 */
function skipAssert(assert: any, count: number): void {
  count || (count = 1)
  while (count--) {
...
(function(): any {
  ...
  function createToString(funcName: string): any{
    return nvStable.constant(nativeString.replace(reToString, funcName))
  }

  // Allow bypassing native checks.
  setProperty(funcProto, 'toString', function wrapper() {
    setProperty(funcProto, 'toString', fnToString)
    const result = nvStable.has(this, 'toString') ? this.toString() : fnToString.call(this)
  ...
  setProperty(objectProto, 'propertyIsEnumerable', function(key: string) {
  ...
  if (Buffer) {
    defineProperty(root, 'Buffer', {
      ...
      'get': function get() {
        ...
        if (!(name == 'runInContext' || name.length == 1 || /\b_\.isBuffer\b/.test(caller as unknown as string))) {
          return Buffer
        }
      }
    })
  ...
  if (Map) {
    setProperty(root, 'Map', (function(): any {
      let count = 0
      return function(): any {
        if (count++) {
          return new Map
  ...
  nvBizarro = interopRequire(filePath)
  root._ = oldDash
...
nvStable.attempt(function() {
  nvStable.assign(realm, require('vm').runInNewContext([
    '(function() {',
    '  var noop = function() {},',
    '      root = this',
    '',
...
nvStable.attempt(function() {
  ...
  let idoc
  idoc = (idoc = iframe.contentDocument || iframe.contentWindow).document || idoc
  idoc.write([
    '<html>',
    '<body>',
    '<script>',
...
nvStable.attempt(async function() {
